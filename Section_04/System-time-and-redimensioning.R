...
# To make Program #2 more efficient, we should
# preallocate memory for x before the loop.
# This version will run much faster because
# R doesn't have to reallocate memory at each step.
slower_fix <- function() {
  n <- 100000
  x <- numeric(n) # Preallocate vector with size n
  x[1] <- 1
  for (i in 2:n) {
    x[i] <- i
  }
